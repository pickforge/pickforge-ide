...
- 2026-06-04: Added `docs/qa/native-host-validation.md` with the macOS/iOS Simulator commands, visible Pickforge steps, pass criteria, and required artifacts needed to close the live iOS VM Service and inspector validation on a macOS host. The validation checkbox remains unchecked until that native-host pass is run.
...
- 2026-06-04: Added `docs/qa/native-host-validation.md` with native macOS desktop and Windows desktop VM Service/inspector dogfood commands, pass criteria, and artifact requirements. The macOS/Windows validation checkbox remains unchecked until those native-host passes are run.
...
- 2026-06-04: Added `docs/qa/native-host-validation.md` and linked it from the manual dogfood and release checklist docs so macOS/iOS Simulator, macOS desktop, and Windows desktop signoff have concrete commands, pass criteria, and required evidence. Native-host release checks remain unchecked until run on those hosts.
...
- 2026-06-04: Added `scripts/linux_installed_visible_smoke.sh` for the remaining post-install Linux desktop pass. It launches an installed `/usr/bin/pickforge` with a clean HOME from a visible desktop, records logs/window evidence/screenshot, and supports `--app <path>` for dry-run validation. Ran the dry run against `build/linux/x64/release/bundle/pickforge` in the visible KDE Wayland session; the screenshot was nonblank and showed the first-run Pickforge window, while `xdotool` could not discover the Wayland window. The true `/usr/bin/pickforge` pass remains unchecked because this shell cannot run passwordless sudo (`sudo -n true` requires a password).
- 2026-06-04: Added `docs/release-blockers.md` as the release blocker register for Linux fresh install, Linux release signing secret, macOS/iOS native dogfood, macOS signing/notarization/Gatekeeper/Sparkle, Windows native dogfood, Windows signing/SmartScreen/package-manager submission, and Cursor/Gemini installed-agent dogfood. This closes the release-checklist requirement to record blockers as follow-up plan items, but does not close the blocker-specific signoff boxes.

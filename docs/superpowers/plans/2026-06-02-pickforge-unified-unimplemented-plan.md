...
**Blocked:** macOS and Windows checklist execution require native macOS/Windows hosts. Linux signoff is still incomplete only for a true sudo install on a fresh VM or clean machine; rootless package extraction/liveness and installed Claude Code/Codex/OpenCode PTY launch/prompt delivery are covered by smoke evidence.

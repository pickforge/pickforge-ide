...
/// from `screencap`; screenshots therefore go through this seam.
typedef AndroidBinaryProcessRunner = Future<ProcessResult> Function(
  String executable,
  List<String> arguments,
);

/// An Android device visible to ADB.
class AndroidAdbDevice extends Equatable {
...
/// Shared ADB support for every Android-backed target (React Native, native
/// Android): device listing, screenshots, UIAutomator dump, and logcat.
class AndroidAdbService {
  AndroidAdbService(
    this._runner,
    this._detector, {
    AndroidBinaryProcessRunner? screencapRunner,
  }) : _screencapRunner = screencapRunner ?? _defaultScreencapRunner;

  final ProcessRunner _runner;
  final BinaryDetector _detector;
  final AndroidBinaryProcessRunner _screencapRunner;
  ...
  Future<List<AndroidAdbDevice>> listDevices() async {
  ...
  ///
  /// The caller OWNS the returned process: pipe its lines through a log parser
  /// and `kill()` it to stop streaming — `adb logcat` runs unbounded otherwise.
  Future<RunningProcess?> streamLogcat({required String serial}) async {
    if (!await _detector.isBinaryOnPath('adb')) return null;
    return _runner.spawn('adb', ['-s', serial, 'logcat', '-v', 'threadtime']);
  }

  List<AndroidAdbDevice> _parseDevices(String output) {
    ...
    for (final raw in output.split('\n')) {
      ...
      devices.add(
        AndroidAdbDevice(serial: serial, state: state, model: model),
      );
    }
    return devices;
  }

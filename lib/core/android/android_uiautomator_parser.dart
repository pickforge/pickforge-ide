...
/// Parses device-side UIAutomator XML into an [AndroidA11yNode] tree and
/// answers hit-test / ancestor queries against it.
///
/// Framework-agnostic: UIAutomator XML is the same for native Views, Compose
/// semantics, and React Native, so this is shared across Android targets.
...
class AndroidUiAutomatorParser {
  const AndroidUiAutomatorParser();
  ...
  AndroidA11yNode parse(String xml) {
    ...
    final children = <AndroidA11yNode>[];
  ...
  AndroidA11yNode? hitTest(AndroidA11yNode root, Offset devicePoint) {
  ...
  List<AndroidA11yNode> ancestorHierarchy(
    AndroidA11yNode root,
    String nodeId,
  ) {
    final path = <AndroidA11yNode>[];

    bool walk(AndroidA11yNode node) {
  ...
  AndroidA11yNode _toNode(XmlElement element, String nodeId) {
    final children = <AndroidA11yNode>[];
  ...
  AndroidA11yRole _role(String className) {
    final c = className.toLowerCase();
    if (c.contains('button')) return AndroidA11yRole.button;
    if (c.contains('edittext')) return AndroidA11yRole.input;
    if (c.contains('image')) return AndroidA11yRole.image;
    if (c.contains('switch')) return AndroidA11yRole.switchControl;
    if (c.contains('checkbox')) return AndroidA11yRole.checkbox;
    if (c.contains('recyclerview') ||
        c.contains('listview') ||
        c.contains('scrollview')) {
      return AndroidA11yRole.list;
    }
    if (c.contains('textview')) return AndroidA11yRole.text;
    return AndroidA11yRole.unknown;
  }

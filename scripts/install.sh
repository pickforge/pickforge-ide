...
write_appimage_wrapper() {
...

  {
    ...
    printf '  return 1\n'
    printf '}\n'
    printf 'has_fuse_device_access() {\n'
    printf '  [ -c "$fuse_device" ] && [ -r "$fuse_device" ] && [ -w "$fuse_device" ]\n'
    printf '}\n'
    printf 'is_known_fuse_restricted_host() {\n'
    printf '  if [ -n "${WSL_DISTRO_NAME:-}" ] || [ -n "${WSL_INTEROP:-}" ]; then\n'
    printf '    return 0\n'
    printf '  fi\n'
    printf '  if [ -f /proc/version ] && grep -qiE '"'"'microsoft|wsl'"'"' /proc/version; then\n'
    printf '    return 0\n'
    printf '  fi\n'
    printf '  if [ -n "${container:-}" ]; then\n'
    printf '    return 0\n'
    printf '  fi\n'
    printf '  if [ -f /.dockerenv ] || [ -f /run/.containerenv ]; then\n'
    printf '    return 0\n'
    printf '  fi\n'
    printf '  if [ -f /proc/1/cgroup ] && grep -qiE '"'"'docker|containerd|kubepods|podman|lxc'"'"' /proc/1/cgroup; then\n'
    printf '    return 0\n'
    printf '  fi\n'
    printf '  return 1\n'
    printf '}\n'
    printf 'if has_fuse2 && has_fuse_device_access && ! is_known_fuse_restricted_host; then\n'

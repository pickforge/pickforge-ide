...
/** Model ids that belong to another agent's static catalog (Claude/Codex).
 * `modelOption` can only validate against a profile's own static catalog, so
 * it is a no-op for providers whose catalog is discovered at runtime (Pi,
 * OMP) — this closes that gap by rejecting ids that are unambiguously
 * another provider's, guarding against a stale/misrouted id bleeding across
 * providers in the composer's model label. */
function foreignStaticModelIds(agentId: string): Set<string> {
  ...
  if (!ids) {
    ...
    for (const agent of AGENTS) {
      ...
      for (const option of agent.models) ids.add(option.id);
    }

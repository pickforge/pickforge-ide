...
void main() {
  test(
    'v2 -> v3 adds emulator columns + run_session_log; backfills manual mode',
    () async {
      final raw = NativeDatabase.memory(
        ...
        ''')
            ...
            ..execute(
              'INSERT INTO project_settings (project_root) '
              "VALUES ('/tmp/empty');",
            );
        },
      );
      ...
      final manual = await db
          .customSelect(
            'SELECT connection_mode FROM project_settings '
            "WHERE project_root='/tmp/manual'",
          )
          .getSingle();

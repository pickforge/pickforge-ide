...
drives the **real** device/run/inspect functionality against a live Android
emulator/handset or iOS simulator.

It is **opt-in**. Nothing here runs in a normal `cargo test` or `bun run vrt`,
and CI without a device stays green. Android and iOS have separate gates:

```sh
PICKFORGE_E2E_SERIAL=emulator-5556   # the adb serial to drive
PICKFORGE_E2E_IOS_UDID=<sim-udid>    # the already-Booted iOS simulator to drive
```
...
### 3. Per-adapter live smokes — two opt-in tiers (#34–#37 + iOS)
...
**Tier A — always-on-with-device** (gated on `PICKFORGE_E2E_SERIAL` for Android
or `PICKFORGE_E2E_IOS_UDID` for iOS).
...
| native-iOS             | `native-ios`     | simctl screenshot → PNG, os_log → events   |
...
# iOS Tier A (macOS, simulator must already be Booted):
PICKFORGE_E2E_IOS_UDID=<sim-udid> \
  cargo test -p pickforge-core --test live_adapters -- --nocapture
```

**Tier B — heavy real-launch** (gated behind the ADDITIONAL `PICKFORGE_E2E_LAUNCH=1`
flag, because a real `flutter run` / `gradle` / `metro` / `xcodebuild` build is slow and flaky).
Android launches an app on the serial, waits for it to **foreground** (via
`dumpsys activity activities` / `pidof`), screenshots + dumps UIAutomator of the
**running** app, then stops it and asserts clean teardown. iOS builds, installs,
launches, screenshots, then terminates the simulator app. Tier B **SKIPS loudly**
...
The iOS heavy smoke builds `fixtures/sample_ios_app` by default with
`xcodebuild -project SampleIosApp.xcodeproj -scheme SampleIosApp -destination id=<udid>`.
Override with `PICKFORGE_E2E_IOS_PROJECT`, `PICKFORGE_E2E_IOS_SCHEME`, and
`PICKFORGE_E2E_IOS_BUNDLE_ID` when running against another project:

```sh
PICKFORGE_E2E_IOS_UDID=<sim-udid> PICKFORGE_E2E_LAUNCH=1 \
  cargo test -p pickforge-core --test live_adapters -- --nocapture --test-threads=1
```
...
group and SIGKILLed (group-wide) on teardown. The iOS build is a bounded
`xcodebuild` invocation; launch/terminate are one-shot `simctl` calls.
...
| `PICKFORGE_E2E_IOS_UDID` unset              | skip (pass)       | n/a                       |
| iOS UDID set, but simulator missing/Shutdown | skip (pass)      | n/a                       |
| iOS UDID set, simulator Booted              | iOS Tier A asserts the simulator | n/a           |
| `PICKFORGE_E2E_LAUNCH=1` (+ serial/UDID)    | adds Tier B (real launch) | n/a                |
...
- iOS smokes require macOS with Xcode command line tools (`xcrun simctl`,
  `xcodebuild`) and a Booted simulator UDID.
- **Tier B only** (`PICKFORGE_E2E_LAUNCH=1`): the relevant SDK/toolchain
  (`flutter` on PATH for the Flutter heavy smoke; `xcodebuild` for iOS) and a
  real buildable project (defaults to `fixtures/sample_flutter_app` or
  `fixtures/sample_ios_app`). Missing → that smoke skips loudly.
...
- **native-iOS** — `fixtures/sample_ios_app` + `live_adapters.rs` Tier A simulator

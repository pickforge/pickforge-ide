...
// The no-remount invariant (receipt-focus retarget / file-select / scope-switch
// must never remount the center terminal/chat host) is NOT unit-tested here.
// A synthetic sibling component standing in for `Workbench.tsx`'s real
// `ChatHostSlot` would pass regardless of whether the actual composed tree
// still keeps the host outside the Source Control pane's reactive scope --
// it doesn't track the production JSX at all, so it can't fail when that JSX
// changes. That invariant is pinned for real in
// tests/vrt/changes-review-surface.spec.ts, which renders the ACTUAL
// Workbench tree (real `ChatHostSlot`, real `AgentChatView`) in a browser and
// asserts the host's own DOM node stays connected (never torn down and
// recreated) across all three interactions.
